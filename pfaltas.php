...
if($_POST>0)
{
    ...
    if(count($_POST) == 3)
    {
        //------------------Acción de alta articulos-----------------------
        $codigo_art=$_POST['codigo_articulo'];
        $precio_articulo = (is_numeric($_POST['precio_articulo']) ? (double)$_POST['precio_articulo'] : 0);
        $descripcion_articulo=$_POST['descripcion_articulo'];
        

        $query="INSERT INTO `articulo`(`id_articulo`, `codigo`, `descripcion`, `precio`) VALUES (NULL,?,?,?)";
        $stmt=$db->prepare($query);
        $stmt->bind_param('ssd',$codigo_art,$descripcion_articulo,$precio_articulo);
        $stmt->execute();
